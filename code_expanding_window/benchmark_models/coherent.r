...
splits <- list(
  c(1959, 1979, 1980, 1989),
  c(1959, 1989, 1990, 1999),
  c(1959, 1999, 2000, 2009),
  c(1959, 2009, 2010, 2019)
)

for (split in splits) {
  train_start <- split[1]; train_end <- split[2]
  test_start  <- split[3]; test_end  <- split[4]
  split_label <- glue("train{train_start}-{train_end}_test{test_start}-{test_end}")
  message("\n=== Split: ", split_label, " ===")

  forecast_years <- test_start:test_end

  file <- glue("country_training_{train_start}_{train_end}.txt")
  ...
  # get all countries whose data does not extend back to train_start
...
write.table(final_fitted_df,
            paste(path, glue("coherent_fitted_{split_label}.txt"), sep = "/"),
            sep = " ", col.names = FALSE, row.names = FALSE)
write.table(final_forecasted_df,
            paste(path, glue("coherent_forecast_{split_label}.txt"), sep = "/"),
            sep = " ", col.names = FALSE, row.names = FALSE)
}
