...
def get_data(index, data, max_val, mode, min_year, max_train_year, forecast_horizon=10, changeratetolog=False):
    if mode == "train":
    ...
    year = (year - min_year) / (max_train_year + forecast_horizon - min_year)
...
def prep_data(data, mode, min_year, max_train_year, forecast_horizon=10, changeratetolog=False):
    ...
    dataset = dataset.map(
        lambda x: get_data(x, data, max_val=max_val, mode=mode,
                           ...
                           changeratetolog=changeratetolog),
