...
# also load test sets and combine for full data
...
for (cutoff in cutoff_years) {
  cat("\n========== Lee-Carter: cutoff =", cutoff, "==========\n")

  # Training data: years up to cutoff
  ...
  for (i in countries) {
    for (j in genders) {
      tryCatch({
        ...
        lc_output <- lca(data, years = years_i, ages = ages, adjust = 'none')
        ...
        forecasted <- forecast(lc_output, h = h)
  ...
  write.table(final_fitted_df,
              paste(path, glue("lc_fitted_cutoff{cutoff}.csv"), sep = "/"),
              sep = ",", col.names = FALSE, row.names = FALSE)
  write.table(final_forecasted_df,
              paste(path, glue("lc_forecast_cutoff{cutoff}.csv"), sep = "/"),
              sep = ",", col.names = FALSE, row.names = FALSE)
