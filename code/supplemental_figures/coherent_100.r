...
country_training <- read.table(paste(input_path, file, sep = "/"), header = FALSE,
...
for (iter in seq_len(n_iter)) {
  ...
  out_file <- glue("coherent_forecast_100_year.csv")
  write.table(final_df, paste(output_path, out_file, sep = "/"), sep = ",", col.names = FALSE, row.names = FALSE)
