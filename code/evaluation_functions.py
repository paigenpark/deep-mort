...
def calculate_error_by_category(forecasted_data, actual_data, feature_index, lograte=False):
...
def calculate_error(forecasted_data, actual_data, lograte=False):
    # Ensure both arrays are sorted by geo, gender, year, and age
    forecasted_data = forecasted_data[np.lexsort((forecasted_data[:, 3], forecasted_data[:, 2], forecasted_data[:, 1], forecasted_data[:, 0]))]
    actual_data = actual_data[np.lexsort((actual_data[:, 3], actual_data[:, 2], actual_data[:, 1], actual_data[:, 0]))]
    ...
    mae = np.mean(np.abs(forecasted_rates - actual_rates))
        
    return mse, rmse, rrmse


# def calculate_error_by_category(val_data, model, feature_index):
#     predictions = []
#     true_values = []
#     feature_values = []
...
#     if feature_index == 0:
#         feature_values = feature_values * 60 + 1959 # reversing normalization, need for year
...
#         rmse = np.sqrt(np.mean(((filtered_preds - filtered_true_values) ** 2)))
#         rrmse = np.sqrt(np.mean((filtered_preds - filtered_true_values) ** 2)) / np.mean(filtered_true_values)
#         # rrmse = np.sqrt(np.mean((filtered_preds - filtered_true_values) ** 2) / np.sum(filtered_preds ** 2))

#         mse_by_category[value] = mse
#         rmses_by_category[value] = rmse
#         rrmses_by_category[value] = rrmse

#     return mse_by_category, rmses_by_category, rrmses_by_category
